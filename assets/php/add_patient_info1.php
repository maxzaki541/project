...
if ($pi6 == "----เลือกคำนำหน้า----") {
  ...
  if ($con->query($sql)) {
    ...
    if ($con->query($sqladd)) {
      $sqlser = "INSERT INTO `service` (se_id,se_idcard,se_idlicense,se_idpublic,se_idhn,se_idfamily,se_idXray,se_roft,emp_id,Info_id)  VALUES('$pi28','$pi0','$pi29','$pi30','$pi31','$pi32','$pi33','$pi34','$pi40','$pi1')";

      if ($con->query($sqlser)) {
        $sqlser2 = "INSERT INTO `service2` (se2_id,se2_pressure,se2_weight,se2_height,se2_waist,se2_temp,emp_id,Info_id,sdate) VALUES('$pi99','$pi35','$pi36','$pi37','$pi38','$pi39','$pi40','$pi1','$date') ";
        if ($con->query($sqlser2)) {
          ...
          if ($con->query($sqldrug)) {
            ...
            $_SESSION['error'] = "เพิ่มข้อมูลไม่สำเร็จเนื่องจากข้อมูลบริการผิดพลาด22 " . $con->error;

            $deladd = "DELETE from `patient_info` where `Info_id` = '$pi1'";
            $con->query($deladd);

            $delaser2 = "DELETE from `service2` where se2_id = '$pi99'";
            $con->query($delaser);
          ...
          $_SESSION['error'] = "เพิ่มข้อมูลไม่สำเร็จเนื่องจากข้อมูลบริการผิดพลาด33 " . $con->error;
      ...
      $_SESSION['error'] = "เพิ่มข้อมูลไม่สำเร็จเนื่องจากข้อมูลที่อยู่ผิดพลาด55 " . $con->error . " " . $pi17;
      $deladd = "DELETE from `patient_info` where `Info_id` = '$pi1'";
      $con->query($deladd);
      header("location:../../add_patient_info.php");
    }
  } else {
    $_SESSION['error'] = "เพิ่มข้อมูลไม่สำเร็จเนื่องจากข้อมูลผิดพลาด666 " . $con->error;
